...
class GetAllGamesTest(TestCase):
	...
	def test_get_all_games(self):
		response = client.get(reverse('games:get_all_games'))
...
class GetSingleGameTest(TestCase):
	...
	def test_get_valid_single_game(self):
		response = client.get(
			reverse('games:get_game_by_id', kwargs={'game_id':self.game2.pk}))
	...
	def test_get_invalid_single_game(self):
		response = client.get(
			reverse('games:get_game_by_id', kwargs={'game_id':30}))
...
class DeleteSingleGameByIDTest(TestCase):
	...
	def test_valid_delete_game(self):
		response = client.delete(
			reverse('games:delete_game_by_id', kwargs={'game_id': self.game1.pk}))
		self.assertEqual(response.status_code, status.HTTP_200_OK)

	def test_invalid_delete_game(self):
		response = client.delete(
			reverse('games:delete_game_by_id', kwargs={'game_id': 30}))
		self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
